...
export const useEditorStore = defineStore("editors", () => {
    ...
    const addEditor = (editor = { input: "", collapsed: false, selected: false }) => {
    ...
    const lintCode = async (editor) =>{
        if (!tauri)
            return;
        try {
            const code = editor.input;
            const result = await invoke("lint_code", {
                luaCode: code,
            });
